...
public static int getRowNum(XlsxReader xls, String id){
		...
		{
		    WebDriver driverObj = (WebDriver)TestBedUtil.currentTestBedInfo().getDriver();
		    if(create)
		    {             
		           driverObj = (WebDriver)DriverManager.buildDriver(TestBedUtil.currentTestBedInfo().getTestBedName()).getDriver();
		        		  // TestBedManager.INSTANCE.getCurrentTestBed().getTestBedName()).getDriver();
		           TestBedUtil.currentTestBedInfo().setDriver(driverObj);
